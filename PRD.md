...
## 14. 性能优化路线图 (2026-01 更新)

为了维持 DASMatrix 的高性能计算特点，针对现有架构中发现的瓶颈，制定以下优化路径：

### 14.1 [P0] 改进 Dask 分块策略 (内存与并行优化)
- **问题**: 现有算子硬编码 `time=-1` 导致大数据量下内存溢出，且破坏了并行性。
- **方案**: 
  - 移除强制 `time=-1` 的逻辑，保留原始 Dask 分块。
  - 对滤波器（Bandpass/Lowpass等）使用 `dask.array.map_overlap` 处理块间边界效应。
  - 确保整个计算链路维持 "Lazy" 特性，直到 `collect()`。

### 14.2 [P1] 实现真延迟 STFT/FK 变换
- **问题**: `stft()` 和 `fk_filter()` 内部调用了 `collect()`，导致核外计算失效。
- **方案**: 
  - 使用 `dask.array.map_blocks` 重构变换类算子。
  - 返回基于 Dask 的延迟 DASFrame，而不是立即计算的 NumPy 数组。

### 14.3 [P1] 可视化自动降采样 (Plot Protection)
- **问题**: 直接绘制 TB 级数据会导致前端或绘图引擎崩溃。
- **方案**: 
  - 在 `plot_heatmap` 和 `plot_ts` 中引入 `max_display_samples` 限制。
  - 对超出限制的数据自动应用 `coarsen` (平均降采样) 或 `decimate` (抽稀)。

### 14.4 [P2] Numba 后端单次扫描优化
- **问题**: `detrend` 和 `demean` 目前采用双倍扫描（先算均值再算处理），浪费内存带宽。
- **方案**: 
  - 在 Numba 内核中实现在线统计算法（如 Welford 算法计算均值/方差）。
  - 实现单次遍历内存完成统计和信号处理融合。
