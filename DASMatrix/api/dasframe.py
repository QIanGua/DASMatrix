...
from ..core.computation_graph import ComputationGraph, Node, SourceNode, OperationNode, NodeDomain, FusionNode
...
class DASFrame:
    ...
    def bandpass(self, low, high, order=4, fs=None, design="butter"):
        """带通滤波。"""
        return self._apply_signal_op("bandpass", low=low, high=high, order=order, fs=fs or self._fs)
    ...
    def highpass(self, cutoff, order=4):
        """高通滤波。"""
        return self._apply_signal_op("highpass", cutoff=cutoff, order=order, fs=self._fs)
    ...
    def stft(self, nperseg=256, noverlap=None):
        """短时傅里叶变换。"""
        if noverlap is None:
            noverlap = nperseg // 2
        return self._apply_signal_op("stft", nperseg=nperseg, noverlap=noverlap, fs=self._fs)
    ...
    # --- 可视化 (Visualization) ---
    def plot_ts(self, ch=None, title="Time Series", **kwargs):
        """绘制时间序列。"""
        data = self.collect()
    ...
    def plot_heatmap(self, title="DAS Waterfall", cmap="RdBu_r", **kwargs):
        """绘制热图（瀑布图）。"""
        data = self.collect()
        fig, ax = plt.subplots(figsize=(10, 6))
        
        extent = [0, data.shape[1], data.shape[0] / self._fs, 0]
    ...
    def plot_spec(self, ch=0, nperseg=256, title="Spectrogram", **kwargs):
        """绘制频谱图。"""
