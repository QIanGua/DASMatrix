...
def create_updated_notebook():
    ...
    add_code(
        [
            ...
            "        # 添加谐波\n",
            "        freq = 2 * signal_freq\n",
            "        term = 0.3 * np.sin(2 * np.pi * freq * (t - phase_delay))\n",
            "        signal += term\n",
            "\n",
            "        # 模拟事件（如地震或入侵）\n",
            "        if 20 < ch < 45:\n",
    ...
    add_markdown(
        [
            ...
            "\n",
            "**注意**：`fft()` 和 `stft()` 方法现在返回 `DASFrame` 对象。",
        ]
    )
    add_code(
        [
    ...
    add_code(
        [
            ...
            "\n",
            "# 检测结果叠加\n",
            "detection_frame = DASFrame(detections.astype(float), fs=frame.fs)\n",
            "detection_frame.plot_heatmap(\n",
            '    title="检测结果", ax=axes[1], cmap="binary"\n',
            ")\n",
            "\n",
            "plt.tight_layout()\n",
            "plt.show()",
